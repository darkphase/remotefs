...
void TestPath::testJoin()
{
	...
	CPPUNIT_ASSERT(static_cast<size_t>(slashed_path_len) == strlen(slashed_dir) + strlen(file));
	CPPUNIT_ASSERT(static_cast<size_t>(slashed_path_len) == strlen(slashed_full_path));
...
void TestPath::testBounds()
{
	...
	full_path = new char[buffer_size];
	CPPUNIT_ASSERT(static_cast<size_t>(path_join(full_path, buffer_size, dir, file)) == buffer_size - 1);
	delete[] full_path;
}
...
void TestPath::testDots()
{
	...
	CPPUNIT_ASSERT(static_cast<size_t>(full_path_len) == strlen(dir) + strlen(dot));
	...
	CPPUNIT_ASSERT(static_cast<size_t>(full_path_len) == strlen(dir) + strlen(dotdot));
	CPPUNIT_ASSERT(memcmp(full_path, dir, strlen(dir)) == 0);
